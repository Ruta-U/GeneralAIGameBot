...
class ProcessFrame:
    def __init__(self, frame_height=84, frame_width=84): 
        #Default 84x84 pixel frame on window for gameplay
        """
           Function constructor() for Process Frame: 
           Resizes and converts RGB Atari frames to grayscale
           (For Easy to Render on Screen)
           Args:
...
class DQN:
    ...
    def __init__(self, n_actions, hidden=1024, learning_rate=0.00001, 
        ...
        self.conv1 = tf.keras.layers.Conv2D(filters=32, kernel_size=[8, 8], strides=4,
            ...
            padding="valid", activation=tf.nn.relu, use_bias=False, name='conv1')(inputs=self.inputscaled)
        self.conv2 = tf.keras.layers.Conv2D(
            filters=64, kernel_size=[4, 4], strides=2, 
            kernel_initializer=tf.variance_scaling_initializer(scale=2),
            padding="valid", activation=tf.nn.relu, use_bias=False, name='conv2')(inputs=self.conv1)
        self.conv3 = tf.keras.layers.Conv2D(
            filters=64, kernel_size=[3, 3], strides=1, 
            kernel_initializer=tf.variance_scaling_initializer(scale=2),
            padding="valid", activation=tf.nn.relu, use_bias=False, name='conv3')(inputs=self.conv2)
        self.conv4 = tf.keras.layers.Conv2D(
            filters=hidden, kernel_size=[7, 7], strides=1, 
            kernel_initializer=tf.variance_scaling_initializer(scale=2),
            padding="valid", activation=tf.nn.relu, use_bias=False, name='conv4')(inputs=self.conv3)
...
class TargetNetworkUpdater:
    ...
    def __init__(self, main_dqn_vars, target_dqn_vars):
        ...
        Args:
            ...
            target_dqn_vars: A list of tensorflow variables belonging to the target DQN
        network"""
...
def train():
    ...
    my_replay_memory = ReplayMemory(size=MEMORY_SIZE, batch_size=BS)   # (imp)
...
if(not os.path.isdir(PATH)) :
    os.makedirs(PATH)
if(not os.path.isdir("./summaries/run_1")) :
